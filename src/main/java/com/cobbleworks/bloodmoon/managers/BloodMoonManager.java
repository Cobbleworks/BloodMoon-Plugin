        ...
        for (World world : getActiveWorlds()) {
            for (Player player : world.getPlayers()) {
                ...
                if (random.nextDouble() < 0.12D) {
                    world.playSound(origin, Sound.AMBIENT_SOUL_SAND_VALLEY_MOOD, 0.30F, 0.60F + random.nextFloat() * 0.2F);
                }
                if (random.nextDouble() < 0.04D) {
                    world.playSound(origin, Sound.ENTITY_WITHER_AMBIENT, 0.25F, 0.55F + random.nextFloat() * 0.2F);
                }
